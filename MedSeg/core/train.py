...
class SegmentationModel3D(pl.LightningModule):
    """
    Model class that implements methods
    """
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self._model.parameters(), config.LR)
...
if __name__ == "__main__":
    net = SegmentationModel3D(unet_baseline)
    ...
    chk_pt = ModelCheckpoint(
        dirpath="models/spleen_version_final_unet/",
        monitor="val_loss",
        save_top_k=2,
        every_n_epochs = 1,
    )
...
#        resume_from_checkpoint="logs/default/version_1/checkpoints/epoch=251-step=2015.ckpt",
    )
